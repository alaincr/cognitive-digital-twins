...
| `ops/outbox_human.jsonl` | task_harvest (B3) | ingest.clj (nouveau reader, même mécanique offset) | `{event: human.response[.amended\|.ambiguous], task_id, task_type, response_text?, choice?, response_hash, answered_ts, provenance:{source:"human"}}` | nouveau (B3) |
...
| `$C/task_causes.jsonl` | ingest.clj | task_gen (B3) | `{cause: contradiction\|triage.due\|bridge, …, event_id}` (annexe B4 §2.4) | nouveau (B4) |
...
| `$C/stance_diff.jsonl` | cycle_tools (B4 ét. 6) | writeback orders, eval E4 | `{node, old_status, new_status, caused_by}` | nouveau (B4) |
